...
\ib{Déclinaison} — [Trad. L. {\it clinamen}*] —
...
\ib{Définition} — [L. {\it finis}, borne, limite]
— \si{Log.} \fsb{S. abstr.} {\bf 1.} Opération qui consiste à
déterminer$^1$ les « limites » de l’extension$^3$ d’un concept, ce qui se fait par
une analyse de sa compréhension$^2$.
$->$ {\it Dist.} description*. — {\bf 2.} \fsb{S. concr.} Proposition par laquelle s’exprime cette
...
\ib{Demande} — \si{Épist.} Ancien nom des postulats* : « Une longue suite de
...
\ib{Démythologiser} — Dépouiller [la religion] de ses éléments mythiques pour
...
\ib{Dénotation} — \si{Log. form.} (Opp. : {\it connotation}*). 
...
\ib{Désir} — \si{Psycho.} {\bf 1.} \fsb{S. subje.} Tendance devenue consciente d'elle-même et
accompagnée de la représentation
de son but (voir  {\it Précis}, Ph. I, p. 469).
— {\bf 2.} \fsb{S. subje.} Aspiration profonde : « La
...
\fsb{S. concr.} Tout caractère$^2$, qualitatif ou quantitatif, qui fait qu'un être, un concept est ce qu'il est. {\it Spéc.}, {\it chez Le
...
« spontanéité naïve » du {\it je}*. —
 {\bf 2.} \fsb{S. abstr.} Action de déterminer$^1$.
...
\ib{Déterminisme} — \si{Épist.} \fsb{S. concr.} {\bf 1.} Ensemble des conditions nécessaires$^2$
d’un phénomène : « Connaître le
déterminisme d’une maladie ». —
\fsb{S. abstr.} {\bf 2.} Principe de la science expérimentale selon lequel il existe entre
...
— \si{Méta.} \fsb{S. norma.} {\bf 3.} Doctrine philosophique selon laquelle tout, dans
...
\ib{Devenir} — \si{Méta.} {\bf 1.} \fsb{S. concr.} Série des changements concrets par lesquels passe
% 53
...
(Wahl). — {\bf 2.} \fsb{S. abstr.} Le changement lui-même : « Sur la continuité d’un certain devenir$^1$, j'ai pris une série de
...
\ib{Devoir (nom)} — \si{Mor.} {\bf 6.} \fsb{S. abstr.} L’obligation$^1$ morale : le devoir est la {\it forme}$^2$
de la loi morale (cf. {\it Catégorique}$^2$)
comme le {\it bien}$^2$ en est la matière$^2$. —
 {\bf 7.} \fsb{S. concr.} Objet de cette obligation : « Le
...
\ib{Diachronique} — \si{Hist. Ling.} (Opp. : {\it synchronique}).
...
\ib{Diffusionisme} — \si{Soc.} \fsb{S. norma.} Théorie
...
— {\it Ext.} {\bf 4.} \fsb{S. subje.} Système de références, point de vue : « La dimension de la morale et la dimension
...
\ib{Discernement (Temps de)} — \si{Ps. métr.} Temps nécessaire à un sujet
% 57
...
\ib{Discursif} — \si{Épist.} (Opp. : {\it intuitif}).
...
\ib{Disponible} — (Opp. : {\it engagé}*). Qualifie {\it auj.} l'attitude de
...
\ib{Distinct} — \si{Vulg.} {\bf 1.} \fsb{S. subje.} Net, précis :
...
\ib{Distributive (Justice)} — \si{Mor.} (Opp. :
...
\ib{Dominateur (Argument)} — [En grec : {\it kurieuôn logos}] — \si{Hist.}
...
\ib{Données} — \si{Math.} {\bf 1.} (Opp. : {\it inconnues}).
...
et {\it pluralisme}*). \fsb{S. norma.} Système métaphysique qui admet dans l’univers deux
